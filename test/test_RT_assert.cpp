...
TEST(TestRTAssert, ShouldFail) {
  ...
  EXPECT_EXIT(
      ...
      ".+[ASSERT].+`i \\* d < 12.0` failed: 3 \\* 4.2 is greater than 12\n"
  );

  EXPECT_EXIT(
      RT_ASSERT(i * d > 13.0, "3 * 4.2 is less than 13");,
      ::testing::KilledBySignal(SIGABRT),
      ".+[ASSERT].+`i \\* d > 13.0` failed: 3 \\* 4.2 is less than 13\n"
  );

  EXPECT_EXIT(
      RT_ASSERT((std::is_same_v<decltype(i * d), int>), "Type of expression should be double");,
      ::testing::KilledBySignal(SIGABRT),
      ".+[ASSERT].+`\\(std::is_same_v<decltype\\(i \\* d\\), int>\\)` failed: Type of expression should be double\n"
  );
  // clang-format on
}
